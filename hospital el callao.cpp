...
bool inicializarSistemaArchivos() {
  ...
  const char* archivos[] = {"pacientes.bin", "doctores.bin", "citas.bin",
                            "historiales.bin"};
...
HistorialMedico obtenerUltimaConsulta(int pacienteID) {
  ...
  if (!encontrada) {
    cout << "Error: No se pudo encontrar la última consulta del paciente"
         << endl;
    ultimaConsulta.id = -1;
  }
...
bool agregarPaciente() {
  ...
  if (existePacienteConCedula(nuevoPaciente.cedula)) {
    cout << "Error: Ya existe un paciente con la cédula "
         << nuevoPaciente.cedula << endl;
    return false;
  }
  ...
  if (archivo.tellp() == 0) {
    ...
    archivo.seekp(0);
    archivo.write(reinterpret_cast<const char*>(&nuevoHeader),
                  sizeof(ArchivoHeader));
  }
  ...
  if (exito) {
    ...
    cout << "  Nombre: " << nuevoPaciente.nombre << " "
         << nuevoPaciente.apellido << endl;
...
bool crearPaciente(const char* nombre, const char* apellido, const char* cedula,
                   int edad, char sexo) {
  // Validaciones iniciales
  ...
  if (strcmp(header.tipoArchivo, "INVALIDO") != 0) {
    ...
    if (actualizarHeader("pacientes.bin", header)) {
      ...
      cout << "Sexo: " << (nuevoPaciente.sexo == 'M' ? "Masculino" : "Femenino")
           << endl;

      // Mostrar fecha de creación formateada
      char fechaStr[20];
      strftime(fechaStr, sizeof(fechaStr), "%Y-%m-%d %H:%M",
               localtime(&nuevoPaciente.fechaCreacion));
      cout << "Registrado: " << fechaStr << endl;
...
bool actualizarPaciente(int id) {
  ...
  if (exito) {
    ...
    cout << "? Fecha de modificación: "
         << formatearFecha(paciente.fechaModificacion) << endl;
  } else {
...
bool agregarPaciente(Paciente nuevoPaciente) {
  ...
  if (existente.id != -1) {
    cout << "Error: Ya existe un paciente con esa cedula "
         << nuevoPaciente.cedula << endl;
    return false;
  }
  ...
  if (strcmp(header.tipoArchivo, "INVALIDO") != 0) {
    ...
    if (actualizarHeader("pacientes.bin", header)) {
      ...
      cout << "  Nombre: " << nuevoPaciente.nombre << " "
           << nuevoPaciente.apellido << endl;
      cout << "  Cédula: " << nuevoPaciente.cedula << endl;
      cout << "  Fecha: " << formatearFecha(nuevoPaciente.fechaCreacion)
           << endl;
      return true;
    }
  }
  ...
  while (archivo.read(reinterpret_cast<char*>(&p), sizeof(Paciente)) &&
    ...
    if (!p.eliminado) {
      ...
      printf("| %4d | %-24s | %-12s | %4d | %10d |\n", p.id, nombreCompleto,
             p.cedula, p.edad, p.cantidadConsultas);
...
void mostrarHistorialMedico(int pacienteID) {
  ...
  if (paciente.eliminado) {
    cout << "Error: No se puede mostrar historial de paciente eliminado."
         << endl;
    return;
  }
...
bool actualizarDoctor(Doctor doctorModificado) {
  ...
  if (strcmp(doctorActual.cedula, doctorModificado.cedula) != 0) {
    Doctor existente = buscarDoctorPorCedula(doctorModificado.cedula);
    if (existente.id != -1) {
...
bool asignarPacienteADoctor(int idDoctor, int idPaciente) {
  ...
  if (doctor.cantidadPacientes >= 50) {
    cout << "Error: El doctor ya tiene el máximo de pacientes asignados (50)"
         << endl;
    return false;
  }
  ...
  if (exito) {
    cout << "Paciente ID " << idPaciente << " asignado al Doctor ID "
         << idDoctor << " correctamente." << endl;
  } else {
...
void listarPacientesDeDoctor(int idDoctor) {
  ...
  for (int i = 0; i < doctor.cantidadPacientes; i++) {
    int idPaciente = doctor.pacientesIDs[i];  // CORREGIDO: pacientesIDs en
                                              // lugar de pacientesAsignados

    // Buscar paciente en archivo
    Paciente paciente = buscarPacientePorID(idPaciente);
    if (paciente.id != -1 && !paciente.eliminado) {
      char nombreCompleto[100];
      snprintf(nombreCompleto, sizeof(nombreCompleto), "%s %s", paciente.nombre,
               paciente.apellido);
  ...
  if (pacientesMostrados == 0) {
    cout << "No se encontraron pacientes activos asignados a este doctor."
         << endl;
  } else {
    cout << "Total de pacientes mostrados: " << pacientesMostrados << endl;
    cout << "Pacientes en lista del doctor: " << doctor.cantidadPacientes
         << endl;
...
bool eliminarDoctor(int id) {
  ...
  if (header.registrosActivos > 0) {
    ...
    if (!actualizarHeader("doctores.bin", header)) {
      cout << "Advertencia: Doctor marcado como eliminado pero header no "
              "actualizado"
           << endl;
      return false;
    }
  ...
  cout << "Fecha de eliminacion: " << formatearFecha(doctor.fechaModificacion)
       << endl;
...
bool atenderCita(int idCita, const char* diagnostico, const char* tratamiento,
  ...
  cita.atendida = true;
  // cita.fechaModificacion = time(nullptr); // ELIMINADO - La estructura Cita
  // no tiene este campo

  char observaciones[200];
  snprintf(observaciones, sizeof(observaciones), "Atendida - Diagnóstico: %s",
           diagnostico);
  ...
  nuevaConsulta.id = headerHistorial.proximoID;
  nuevaConsulta.pacienteID =
      cita.idPaciente;  // CORREGIDO: pacienteID en lugar de idPaciente
  ...
  strncpy(nuevaConsulta.diagnostico, diagnostico,
          sizeof(nuevaConsulta.diagnostico) - 1);
  strncpy(nuevaConsulta.tratamiento, tratamiento,
          sizeof(nuevaConsulta.tratamiento) - 1);
  strncpy(nuevaConsulta.medicamentos, medicamentos,
          sizeof(nuevaConsulta.medicamentos) - 1);
  ...
  archivoHistorial.write(reinterpret_cast<const char*>(&nuevaConsulta),
                         sizeof(HistorialMedico));
  ...
  paciente.cantidadConsultas++;
  paciente.fechaModificacion = time(nullptr);
  actualizarPacienteEnArchivo(
      paciente);  // CORREGIDO: usar actualizarPacienteEnArchivo
  ...
  cout << "Consulta agregada al historial médico. ID Consulta: "
       << nuevaConsulta.id << endl;
...
bool agendarCita(int idPaciente, int idDoctor, const char* fecha,
  ...
  if (archivoCitas.is_open()) {
    ...
    for (int i = 0; i < header.cantidadRegistros; i++) {
      Cita citaExistente;
      if (archivoCitas.read(reinterpret_cast<char*>(&citaExistente),
                            sizeof(Cita))) {
        if (!citaExistente.eliminado && citaExistente.idDoctor == idDoctor &&
  ...
  if (archivoCitas2.is_open()) {
    ...
    for (int i = 0; i < header.cantidadRegistros; i++) {
      Cita citaExistente;
      if (archivoCitas2.read(reinterpret_cast<char*>(&citaExistente),
                             sizeof(Cita))) {
...
Cita* obtenerCitasPorFecha(const char* fecha, int* cantidad) {
  ...
  *cantidad = citasEncontradas;
  ...
  cout << "Encontradas " << citasEncontradas << " citas para la fecha " << fecha
       << endl;
...
void listarCitasPendientes() {
  ...
  cout << "\n================================================================"
       << endl;
  cout << "               CITAS PENDIENTES - TOTAL: " << cantidadPendientes
       << endl;
  cout << "================================================================"
       << endl;
  ...
  for (int i = 0; i < cantidadPendientes; i++) {
    ...
    cout << "  Doctor: "
         << (doctor.id != -1
                 ? "Dr. " + string(doctor.nombre) + " " + doctor.apellido
                 : "No encontrado")
         << endl;
    cout << "  Especialidad: "
         << (doctor.id != -1 ? doctor.especialidad : "N/A") << endl;
    cout << "  Motivo: " << c.motivo << endl;
    cout << "  Costo: $" << (doctor.id != -1 ? doctor.costoConsulta : 0)
         << endl;
    cout << endl;
  }
...
bool verificarDisponibilidad(int idDoctor, const char* fecha,
                             const char* hora) {
  // 1. Validar parámetros
  if (!fecha || !hora) {
    cout << "Error: Parámetros inválidos." << endl;
    ...
    switch (opcion) {
      ...
      case 2: {
        ...
        if (paciente.id != -1) {
          ...
          cout << "Consultas realizadas: " << paciente.cantidadConsultas
               << endl;
        } else {
          cout << "Paciente no encontrado." << endl;
        }
      ...
      case 4: {
        ...
        int cantidad =
            buscarPacientesPorNombre(nombre, resultados, MAX_RESULTADOS);
      ...
      case 10: {
        ...
        if (paciente.id != -1) {
          ...
          strftime(fechaCreacion, sizeof(fechaCreacion), "%Y-%m-%d %H:%M",
                   localtime(&paciente.fechaCreacion));
          strftime(fechaModificacion, sizeof(fechaModificacion),
                   "%Y-%m-%d %H:%M", localtime(&paciente.fechaModificacion));

          cout << "Fecha de registro: " << fechaCreacion << endl;
          cout << "Ultima modificacion: " << fechaModificacion << endl;
          cout << "Estado: " << (paciente.eliminado ? "ELIMINADO" : "ACTIVO")
               << endl;
        }
        break;
      }
    ...
    switch (opcion) {
      ...
      case 2: {
        ...
        if (doctor.id != -1) {
          ...
          cout << "ID: " << doctor.id << endl;
          cout << "Nombre: Dr. " << doctor.nombre << " " << doctor.apellido
               << endl;
          cout << "Cedula: " << doctor.cedula << endl;
          cout << "Especialidad: " << doctor.especialidad << endl;
          cout << "Experiencia: " << doctor.aniosExperiencia << " anos" << endl;
      ...
      case 4: {
        ...
        int cantidad = buscarDoctoresPorEspecialidad(especialidad, resultados,
                                                     MAX_RESULTADOS);

        if (cantidad > 0) {
          cout << "\nENCONTRADOS " << cantidad << " DOCTORES EN "
               << especialidad << ":" << endl;
          for (int i = 0; i < cantidad; i++) {
            cout << i + 1 << ". Dr. " << resultados[i].nombre << " "
                 << resultados[i].apellido << " (ID: " << resultados[i].id
                 << ", Experiencia: " << resultados[i].aniosExperiencia
                 << " anos" << ", Costo: $" << resultados[i].costoConsulta
                 << ")" << endl;
          }
        } else {
      ...
      case 5: {
        ...
        Doctor doctor = buscarDoctorPorID(id);
        if (doctor.id != -1) {
          cout << "Actualizando: Dr. " << doctor.nombre << " "
               << doctor.apellido << endl;
          ...
          cout << "Nuevos anos de experiencia (actual: "
               << doctor.aniosExperiencia << "): ";
          cin.getline(buffer, sizeof(buffer));
          if (strlen(buffer) > 0) {
    ...
    switch (opcion) {
      ...
      case 4: {
        ...
        if (citas && cantidad > 0) {
          ...
          cout << "ID  | Fecha      | Hora  | Doctor | Estado    | Motivo"
               << endl;
          cout << "----|------------|-------|--------|-----------|-------------"
                  "------"
               << endl;
          ...
          for (int i = 0; i < cantidad; i++) {
            ...
            cout << setw(3) << c->id << " | " << setw(10) << c->fecha << " | "
                 << setw(5) << c->hora << " | " << setw(6) << c->idDoctor
                 << " | " << setw(9) << c->estado << " | "
                 << (strlen(c->motivo) > 15
                         ? string(c->motivo).substr(0, 12) + "..."
                         : c->motivo)
                 << endl;
          }
      ...
      case 5: {
        ...
        if (citas && cantidad > 0) {
          ...
          cout << "ID  | Fecha      | Hora  | Paciente | Estado    | Motivo"
               << endl;
          cout << "----|------------|-------|----------|-----------|-----------"
                  "--------"
               << endl;
          ...
          for (int i = 0; i < cantidad; i++) {
            ...
            cout << setw(3) << c->id << " | " << setw(10) << c->fecha << " | "
                 << setw(5) << c->hora << " | " << setw(8) << c->idPaciente
                 << " | " << setw(9) << c->estado << " | "
                 << (strlen(c->motivo) > 15
                         ? string(c->motivo).substr(0, 12) + "..."
                         : c->motivo)
      ...
      case 6: {
        ...
        if (citas && cantidad > 0) {
          cout << "\nCITAS PARA " << fecha << " (Total: " << cantidad
               << "):" << endl;
          cout << "ID  | Hora  | Paciente | Doctor | Estado    | Motivo"
               << endl;
          cout << "----|-------|----------|--------|-----------|---------------"
                  "----"
               << endl;
          ...
          for (int i = 0; i < cantidad; i++) {
            cout << setw(3) << citas[i].id << " | " << setw(5) << citas[i].hora
                 << " | " << setw(8) << citas[i].idPaciente << " | " << setw(6)
                 << citas[i].idDoctor << " | " << setw(9) << citas[i].estado
                 << " | "
                 << (strlen(citas[i].motivo) > 15
                         ? string(citas[i].motivo).substr(0, 12) + "..."
                         : citas[i].motivo)
                 << endl;
          }
          delete[] citas;
  ...
  do {
    ...
    switch (opcion) {
      ...
      case 2: {
        ...
        int idPaciente, idDoctor;
        char fecha[11], hora[6], diagnostico[200], tratamiento[200],
            medicamentos[150];
        float costo;
  ...
  do {
    ...
    switch (opcion) {
      case 1: {
        ...
        cout << "====================" << endl;
        cout << "Pacientes registrados: " << headerPacientes.registrosActivos
             << endl;
        cout << "Doctores registrados: " << headerDoctores.registrosActivos
             << endl;
        cout << "Citas en sistema: " << headerCitas.registrosActivos << endl;
        cout << "Consultas en historial: " << headerHistorial.registrosActivos
             << endl;
      ...
      case 3: {
        ...
        for (int i = 0; i < cantidad; i++) {
          ...
          if (!encontrado && totalEspecialidades < MAX_ESPECIALIDADES) {
            strcpy(especialidades[totalEspecialidades],
      ...
      case 5: {
        ...
        cout << "Promedio por consulta: $"
             << (consultasCount > 0 ? ingresosTotales / consultasCount : 0)
             << endl;
      ...
      case 6: {
        ...
        if (archivoCitas.is_open()) {
          ...
          for (int i = 0; i < headerCitas.cantidadRegistros; i++) {
            if (archivoCitas.read(reinterpret_cast<char*>(&cita),
                                  sizeof(Cita))) {
              if (!cita.eliminado && strcmp(cita.estado, "AGENDADA") == 0) {
        ...
        for (int i = 0; i < limite; i++) {
          cout << i + 1 << ". Dr. " << doctores[i].nombre << " "
               << doctores[i].apellido << " (" << doctores[i].especialidad
      ...
      case 7: {
        ...
        for (int i = 0; i < cantidad - 1; i++) {
          for (int j = 0; j < cantidad - i - 1; j++) {
            if (pacientes[j].cantidadConsultas <
                pacientes[j + 1].cantidadConsultas) {
  ...
  do {
    ...
    switch (opcion) {
      ...
      case 5: {
        ...
        cout << "Esta operacion eliminara permanentemente los registros "
