...
bool OperacionesDoctores::agregarDoctor(Doctor& doctor) {
    ...
    // Determinar si el archivo existe o no
    ifstream testFile("doctores.bin", ios::binary);
    bool archivoExiste = testFile.is_open();
    testFile.close();

    // Variables para el header
    ArchivoHeader header;
    bool nuevoArchivo = !archivoExiste;
    int proximoID = 1; // Valor por defecto para nuevo archivo

    if (archivoExiste) {
        // Si el archivo existe, leer el header
        header = GestorArchivos::leerHeader("doctores.bin");
        if (strcmp(header.tipoArchivo, "INVALIDO") == 0) {
            // Si no se puede leer, crear nuevo header
            cout << "Advertencia: Header inválido, creando nuevo archivo..." << endl;
            nuevoArchivo = true;
        } else {
    ...
    // Asignar ID al doctor
    doctor.setId(proximoID);

    // Abrir archivo en modo append (creará si no existe)
    ...
    // Si es un archivo nuevo, escribir el header primero
    if (nuevoArchivo) {
        archivo.close(); // Cerrar para reabrir en modo write
        ...
        // Abrir en modo truncate para crear archivo limpio
        archivo.open("doctores.bin", ios::binary | ios::trunc);
        if (!archivo.is_open()) {
            cout << "Error: No se puede crear archivo de doctores" << endl;
            return false;
        }
        ...
        cout << "Nota: Archivo creado exitosamente" << endl;
        
        archivo.close();
        return true;
    } else {
        // Archivo ya existía, solo agregar el registro
        archivo.write(reinterpret_cast<const char*>(&doctor), sizeof(Doctor));
        ...
        if (exito) {
            ...
            // Actualizar el header en el archivo
            fstream archivoUpdate("doctores.bin", ios::binary | ios::in | ios::out);
            if (archivoUpdate.is_open()) {
                archivoUpdate.seekp(0);
                archivoUpdate.write(reinterpret_cast<const char*>(&header), sizeof(ArchivoHeader));
                archivoUpdate.close();
            }
